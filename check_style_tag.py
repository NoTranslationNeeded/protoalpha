...
print(f"\nFound {len(results)} cards with this style tag (showing first 50):\n")
...
for name, grp_id, art_id, exp, tags in results:
    by_expansion[exp].append((name, grp_id, art_id, tags))
...
for exp in sorted(by_expansion.keys()):
    ...
    for name, grp_id, art_id, tags in cards[:10]:  # Show max 10 per set
        # Count total tags
        tag_count = len(tags.split(',')) if tags else 0
        has_parallax = '1696804317' in tags
        parallax_str = " + Parallax" if has_parallax else ""
        
        print(f"  {name[:40]:40} (GrpId:{grp_id}, {tag_count} tags{parallax_str})")
